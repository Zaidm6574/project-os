...
3. For a new named run, choose an unused slug and scaffold it with `new-run` before writing; retain an existing selected root when resuming. Write `<run-root>/00-project-goal.md` (canonical goal, why, target user, Definition of Done, non-goals) and, when the full-engine roster is available, record the goal hash in `<run-root>/21-agent-roster.md`. For starter-only runs, record tier/scope in the goal and plan; do not assume the roster exists.
