...
try:
    import brain_paths
    from secret_patterns import secret_reason
except ModuleNotFoundError as exc:
    # A copied or partially installed script must refuse before it can write a
    # brain record.  Falling back to a shadowed addon would weaken the shared
    # secret gate; crashing would leave callers unable to distinguish that
    # safety refusal from an internal failure.
    brain_paths = None
    secret_reason = None
...
SHARED_BRAIN = (str(brain_paths.resolve_shared_brain(ROOT))
                if brain_paths is not None else
                os.environ.get("PROJECT_OS_SHARED_BRAIN", ""))
...
def _existing_ids(path):
    """Read dedupe keys while the caller owns the matching brain lock."""
    ids = set()
    try:
        with open(path, "rb") as brain:
            for raw_line in brain:
                try:
...
def main():
    ...
    if _DEPENDENCY_ERROR:
        print("REFUSED: brain_append requires its canonical privacy/path "
              f"modules ({_DEPENDENCY_ERROR})", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        record = json.loads(line, parse_constant=reject_non_finite)
    ...
    # Every durable writer uses the same recursively exhaustive denylist.  It
    # lives in scripts/ so a freshly installed project can enforce this gate
    # without depending on the optional full-engine brain addon being present.
    if not isinstance(record, dict):
        print("REFUSED: shared-brain.jsonl takes one JSON OBJECT per line; "
              f"got a bare {type(record).__name__}.", file=sys.stderr)
        sys.exit(2)
    # Warn from the supplied shape, before normalization hides the operator's
    # alias or missing id.  The writer below still emits the canonical form so
    # every reader receives a valid durable record.
    ...
    if _missing:
        ...
        print("WARNING: record has no nonempty %s%s. It will be normalized "
              "before append; use canonical id/type/text fields to keep "
              "shared-brain records explicit."
              % (", ".join(_missing), _hint), file=sys.stderr)

    try:
        record = brain_paths.canonicalize_record(record)
    except brain_paths.BrainRecordError as exc:
        print(f"REFUSED: {exc}", file=sys.stderr)
        sys.exit(2)
    field = secret_reason(record)
    if field:
        print(
            f"REFUSED: {field} looks like it contains a secret. Redact it and "
            "retry — the shared brain syncs to the central brain and must never "
            "carry credentials.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    token = bb_lock.acquire(SHARED_BRAIN, agent=agent, wait=10)
    if not token:
