...
- **Loop artifacts are files, not claims.** Every loop artifact has an on-disk home you can list with a shipped command: plans `python3 scripts/plan_artifact.py list` (`blackboard/plans/*.json`), evolution records `python3 scripts/evolution.py report --run <name>` (`runs/<run>/evolution.json`), live locks `python3 scripts/bb_lock.py status` (`~/.project-os/locks`), brain gauge `python3 scripts/brain_scale.py`, nightly drift `blackboard/22-automation-log.md`. If a loop artifact doesn't show up in one of those, it isn't real — a session saying it recorded something is not evidence that it did. (A local dashboard, if you have one, is only a view over these files; it is not part of this template.)
...
- **Claude** additionally exposes each as a slash command (`/kickoff`, `/status`, …). Those live in `addons/full-engine/staged/commands/` and are **generated** from `prompts/workflows/` — never hand-edit them. Install with `./install.sh <target> --claude-engine`.
- **Codex** additionally exposes each as a project-local skill in `.agents/skills/<name>/SKILL.md`, generated into `addons/full-engine/staged/codex-skills/`. Install with `./install.sh <target> --codex-engine`. Verified against codex-cli 0.144.1: those skills are auto-loaded at startup, and Codex expands the same `$ARGUMENTS` token Claude does. Both host adapters are optional and equal — neither runtime is the privileged one.
